...
pub async fn find_user_by_username_login(
    username: &str,
) -> Result<UserPasswordResponse, Box<dyn Error>> {
    ...
    match client.query_single::<User, _>(query, &(username,)).await {
        Ok(Some(user)) => Ok(UserPasswordResponse {
...
pub async fn find_user_by_username(username: &str) -> Result<UserResponse, Box<dyn Error>> {
    ...
    // Execute the query
    let user_option: Option<User> = client.query_single(query, &(username,)).await?;
